...
for node_idx in range(processed_dict["locations"].shape[1]):
    ...
    x=processed_dict["locations"][frame_bounds[0]:min(frame_bounds[1],processed_dict["locations"].shape[0]),node_idx,0,:]
    y=processed_dict["locations"][frame_bounds[0]:min(frame_bounds[1],processed_dict["locations"].shape[0]),node_idx,1,:] 
    x_diff_2 = itol.GetDiff(itol.GetDiff(x))
    y_diff_2 = itol.GetDiff(itol.GetDiff(y))
    f_axis = range(len(x_diff_2))
    ax[node_idx//2, node_idx%2].plot(f_axis, x_diff_2, color="blue", linestyle='--', label="x accel")        
    ax[node_idx//2, node_idx%2].plot(f_axis, y_diff_2, color="red", linestyle='--', label="y accel")
plt.legend()
plt.show()
