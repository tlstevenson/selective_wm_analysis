# -*- coding: utf-8 -*-
"""
Created on Fri Apr 10 13:48:19 2026
...
# ---------------------------------------------------------
# 2. Universal HDF5 Loading Logic (SLEAP & DLC)
# ---------------------------------------------------------
def load_tracking_data(filepath):
    """
    Universally loads either SLEAP or DeepLabCut h5 data.
    Returns: locations (frames, nodes, 2) and a list of node_names.
    """
    # 1. Determine the file type by peaking inside
    is_sleap = False
    try:
        ...
        pass # It might be a purely pandas-formatted PyTables HDF5

    # 2. Process SLEAP Format
    if is_sleap:
        with h5py.File(filepath, "r") as f:
    ...
    # 3. Process DeepLabCut Format
    else:
        df = pd.read_hdf(filepath)
        
        # Strip the scorer level if it exists
        if 'scorer' in df.columns.names:
            df.columns = df.columns.droplevel('scorer')
            
        # Handle Multi-Animal DLC (maDLC) if individuals are present
        if 'individuals' in df.columns.names:
            first_individual = df.columns.get_level_values('individuals')[0]
            df = df.xs(first_individual, level='individuals', axis=1)
            print(f"Warning: Multiple individuals found in DLC file {filepath}. Defaulting to {first_individual}.")
            
        # Extract unique bodyparts (nodes)
        node_names = list(df.columns.get_level_values('bodyparts').unique())
        
        num_frames = len(df)
        num_nodes = len(node_names)
        
        # Build the (frames, nodes, 2) array expected by the downstream logic
        locations = np.zeros((num_frames, num_nodes, 2))
        
        for i, node in enumerate(node_names):
            locations[:, i, 0] = df[(node, 'x')].values
            locations[:, i, 1] = df[(node, 'y')].values
...
def process_all_videos(config):
    ...
    for video in config:
        ...
        locations, node_names = load_tracking_data(video["filepath"])
        ...
        for node_idx, node_name in enumerate(node_names):
            ...
            all_records.append({"Group": video["group"], "Node": node_name, "Metric": "Proportion NaNs", "Value": node_data["prop_nans"]})
...
def plot_distributions_all_nodes(df, stats_df):
    ...
    stats_df.round(4).to_csv(csv_filename, index=False)
