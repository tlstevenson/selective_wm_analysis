...
def CubicInterpolation(series, max_dist):
    ...
    for node in range(data_shape[1]):  # Node
        for i in range(2):  # 2d points
            ...
            interpolated_series[:, node, i, 0] = my_series.interpolate(
                method="pchip",
                order=3,
                limit=max_dist,
                limit_area="inside",
...
def extract_thresholded_sequences(convolved_data, thresholds):
    ...
    def get_1d_sequences(signal_1d, thresh):
        ...
        padded = np.pad(
            exceeds, (1, 1), mode="constant", constant_values=False
        )
        ...
        ends = np.where(diffs == -1)[
            0
        ]  # Exclusive end index (standard Python slice logic)
    ...
    for thresh in thresholds:
        if is_2d:
            ...
            for n_idx in range(convolved_data.shape[1]):
                node_results[n_idx] = get_1d_sequences(
...
def threshold_and_interpolate_combinations(
    ...
    interpolated_results["Raw Data"] = CubicInterpolation(tracks, max_frames)
    ...
    for combo_size in range(1, len(method_keys) + 1):
        for combo in combinations(method_keys, combo_size):
            ...
            interpolated_results[combo_name] = CubicInterpolation(
...
r"""label_paths = [
    rf"C:\Users\cns-th-lab\TannerVidsRenamed\{rat}\Videos\predictions\260523_198_199x_237x_238x_274x_400x_402x_424x_483x"
    for rat in active_rats
]"""
port_label_paths = [
...
with h5py.File(label_files[0], "r") as f:
    ...
    project_dict["edge_inds"] = [
        [
            project_dict["node_names"].index(name_1),
...
with h5py.File(get_port_file(label_files[0], port_label_paths), "r") as g:
    project_dict["port_names"] = [
        n.decode("utf-8") for n in g["node_names"][:]
    ]

#%%%% Extract session specific data
labels_dict = {"vid_path":[], "sess": [], "scores": [], "tracks": [], "port_tracks": [], "model_name": []}
...
for file in label_files:
    with h5py.File(file, "r") as f:
        ...
        try:
            ...
            print(e)
            if get_port_file(file, port_label_paths) == None:
                #TODO: Remove this bypassing of errors in the next block
                print("No port file found and copy list added!!!")
                port_names = project_dict["port_names"]
        ...
        # Get Video Location TODO: Eventually will need to integrate with db
        ...
        vid_path = os.path.splitext(vid_path)[0] + ".mp4"
        ...
        labels_dict["vid_path"].append(vid_path)
        labels_dict["model_name"].append(os.path.basename(os.path.dirname(file)))
...
labels_df[["score_thresh_tracks", "score_nan_mask"]] = labels_df.apply(
    lambda row: ThresholdedPositions(
...
labels_df[["edge_thresh_tracks", "edge_nan_mask"]] = labels_df.apply(
    lambda row: calculate_edge_outliers(
        row["rotated_tracks"], edge_names, node_names, iqr_multiplier=1.5
    ),
    axis=1,
    result_type="expand",
)
...
def find_interpolated_sequences(pre_coords, post_coords, node_names=[]):
    ...
    for n_idx in range(n_nodes):
        ...
        padded = np.pad(
            interp_mask, (1, 1), mode="constant", constant_values=False
        )
        diffs = np.diff(padded.astype(int))
        ...
        # Gather intervals specifically for this node
        node_intervals = []
        for s, e in zip(starts, ends):
            node_intervals.append((s, e))
...
dropped_sig_s = first_video_row["tracks"]
dropped_sig_s[first_video_row["score_nan_mask"]] = np.nan
dropped_sig_v = first_video_row["tracks"]
dropped_sig_v[first_video_row["vel_nan_mask"]] = np.nan
dropped_sig_i = first_video_row["tracks"]
dropped_sig_i[first_video_row["iso_nan_mask"]] = np.nan
dropped_sig_s_v = first_video_row["tracks"]
dropped_sig_s_v[first_video_row["score_nan_mask"] | first_video_row["vel_nan_mask"]] = np.nan
dropped_sig_s_i = first_video_row["tracks"]
dropped_sig_s_i[first_video_row["score_nan_mask"] | first_video_row["iso_nan_mask"]] = np.nan
dropped_sig_v_i = first_video_row["tracks"]
dropped_sig_v_i[first_video_row["vel_nan_mask"] | first_video_row["iso_nan_mask"]] = np.nan
dropped_sig_s_v_i = first_video_row["tracks"]
dropped_sig_s_v_i[first_video_row["vel_nan_mask"] | first_video_row["iso_nan_mask"] | first_video_row["score_nan_mask"]] = np.nan


interp_sig_r = CubicInterpolation(first_video_row["tracks"], interpolation_frame_limit)
interp_sig_s = CubicInterpolation(dropped_sig_s, interpolation_frame_limit)
interp_sig_v = CubicInterpolation(dropped_sig_v, interpolation_frame_limit)
interp_sig_i = CubicInterpolation(dropped_sig_i, interpolation_frame_limit)
interp_sig_s_v = CubicInterpolation(dropped_sig_s_v, interpolation_frame_limit)
interp_sig_s_i = CubicInterpolation(dropped_sig_s_i, interpolation_frame_limit)
interp_sig_v_i = CubicInterpolation(dropped_sig_v_i, interpolation_frame_limit)
interp_sig_v_s_i = CubicInterpolation(dropped_sig_s_v_i, interpolation_frame_limit)
...
def find_qualified_static_gaps(coords, max_flank_dist=50.0):
    ...
    for n_idx in range(n_nodes):
        ...
        padded = np.pad(
            nan_mask, (1, 1), mode="constant", constant_values=False
        )
        diffs = np.diff(padded.astype(int))
...
# %%%
hist_edge(
    edge_names, node_names, np.array(labels_df["rotated_tracks"].iloc[0])
)
...
for i, node in enumerate(node_names):
    ...
    kde_mask = detect_outliers_kde(
        node_data, percentile_threshold=1.0, bandwidth=2.0
    )
