...
# -*- coding: utf-8 -*-
"""
Stage 6- nosepoke delay 

@author: ankitakarthik
"""
...
# fill in cpoke in time where it is nan
all_sess.loc[np.isnan(all_sess['cpoke_in_time']), 'cpoke_in_time'] = 0 

#remove trials with no cpoke_out_time
all_sess = all_sess.dropna(subset=['cpoke_out_time'])

# %% Format Data

# calculate nosepoke delay 
all_sess['poke_dur'] = all_sess['cpoke_out_time'] - all_sess['cpoke_in_time']
...
bin_size = 1
poke_bin_max = np.ceil(np.max(all_sess['poke_dur'])/bin_size)
poke_bin_min = np.floor(np.min(all_sess['poke_dur'])/bin_size)
poke_bins = np.arange(poke_bin_min, poke_bin_max+1)*bin_size
poke_bin_labels = ['{:.1f}-{:.1f}s'.format(poke_bins[i], poke_bins[i+1]) for i in range(len(poke_bins)-1)]

all_sess['poke_dur_bin'] = all_sess['poke_dur'].apply(lambda x: poke_bin_labels[np.where(x >= poke_bins)[0][-1]])
...
dur_bin_labels = ['{:.1f}-{:.1f}s'.format(dur_bins[i], dur_bins[i+1]) for i in range(len(dur_bins)-1)]
...
count_columns = ['correct_port', 'stim_dur_bin', 'poke_dur_bin', 'tone_info_str']
...
bah.plot_counts(count_dict['poke_dur_bin'], axs[2], 'Poke Duration', '# Trials', 'h')
...
bah.plot_counts(count_dict_pct['stim_dur_bin'], axs[1], 'Stimulus Duration', '% Trials', 'v')
bah.plot_counts(count_dict_pct['poke_dur_bin'], axs[2], 'Poke Duration', '% Trials', 'v')
