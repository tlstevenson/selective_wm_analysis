...
sess_ids = bah.limit_sess_ids(sess_ids, 6)
...
# also handle bug with missing cpoke out
...
for subj_id in subj_ids:
    ...
    n_stay_reward_choice = {p: {'k': 0, 'n': 0} for p in choice_probs}
    ...
    for sess_id in sess_ids[subj_id]:
        ...
        choice_prob = ind_sess['choice_prob'].to_numpy()[:-1]
...
for subj_id in subj_ids:
    ...
    # logistic regression of stay/switch by prior rewards?
    n_back = 4
    ...
    if include_winstay:
        predictor_labels = utils.flatten([['choice ({})'.format(i), 'reward ({})'.format(i), 
                             'win-stay/lose-switch ({})'.format(i)] for i in range(-1, -n_back-1, -1)])
    else:
        predictor_labels = utils.flatten([['choice ({})'.format(i), 'reward ({})'.format(i), 
                             'choice x reward ({})'.format(i)] for i in range(-1, -n_back-1, -1)])
    ...
    for sess_id in sess_ids[subj_id]:
        ...
        # choices need to be binary for the outcome
        choice_mat.append(choices[1:].copy())
        
        # reformat choices to be -1/+1 for right/left predictors
        choices[choices == 0] = -1
        # get win-stay/lose_switch
        winstay = rewarded.copy()
        winstay[winstay == 0] = -1
        winstay = winstay * choices
        # make buffered predictor vectors
        ...
        if include_winstay:
            sess_mat = np.hstack([np.hstack([buff_choices[i:-n_back+i], buff_reward[i:-n_back+i], buff_winstay[i:-n_back+i]]) 
                                  for i in range(n_back-1, -1, -1)])
        else:
            sess_mat = np.hstack([np.hstack([buff_choices[i:-n_back+i], buff_reward[i:-n_back+i], buff_interaction[i:-n_back+i]]) 
                                  for i in range(n_back-1, -1, -1)])
    ...
    model = sm.Logit(choice_mat, predictors)
    fit_res = model.fit()
