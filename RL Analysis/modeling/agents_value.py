...
class QValueAgent(ModelAgent):
    ...
    def __init__(self, alpha_same=None, alpha_diff=None, alpha_avg=None, v_init=None, k_diff=0, decay_to_avg=False, constraints=None):
        ...
        self.alpha_same = UnitParam(alpha_same)
        self.alpha_diff = UnitParam(alpha_diff)
        self.alpha_avg = UnitParam(alpha_avg)
        self.v_init = PositiveParam(v_init)
        
        self.k_diff = _init_param(k_diff)
        
        self.decay_to_avg = decay_to_avg
    ...
    def apply_constraints(self):
        if self.decay_to_avg:
            self.constraints['k_diff'] = {'fit': False}
    ...
    def reset_params(self):
        self.alpha_same = UnitParam()
        self.alpha_diff = UnitParam()
        self.alpha_avg = UnitParam()
        self.v_init = PositiveParam()
        
        self.k_diff = _init_param(0)
    ...
    def reset_state(self, n_sess=1):
        # initialize state
        self.state = torch.cat([self.v_init.a, self.v_init.a]).unsqueeze(0).expand(n_sess, -1)
        self.avg_val = self.v_init.a.unsqueeze(0).expand(n_sess, -1)
        self.state_hist = []
        self.avg_val_hist = []
        self.state_diff_hist = []
        self.state_delta_hist = []
        self.state_hist.append(self.state.detach())
        self.avg_val_hist.append(self.avg_val.detach())
    ...
    def step(self, input):
        chosen_side_idxs = input[:,1].type(torch.long).unsqueeze(1)
        unchosen_side_idxs = input[:,0].type(torch.long).unsqueeze(1)
        
        # isolate chosen/unchosen sides
        chosen_val = self.state.gather(dim=1, index=chosen_side_idxs)
        unchosen_val = self.state.gather(dim=1, index=unchosen_side_idxs)
        
        if self.decay_to_avg:
            unchosen_set = self.avg_val
        else:
            unchosen_set = self.k_diff.unsqueeze(0)

        chosen_diffs = input[:,2].unsqueeze(1) - chosen_val
        unchosen_diffs = unchosen_set - unchosen_val

        # update state
        chosen_deltas = chosen_diffs * self.alpha_same.a
        unchosen_deltas = unchosen_diffs * self.alpha_diff.a
        
        # switch the order back into left/right side
        state_diffs = torch.cat([chosen_diffs, unchosen_diffs], dim=1)
        state_deltas = torch.cat([chosen_deltas, unchosen_deltas], dim=1)
        
        gather_idxs = torch.cat([chosen_side_idxs, unchosen_side_idxs], dim=1)
        state_diffs = state_diffs.gather(dim=1, index=gather_idxs)
        state_deltas = state_deltas.gather(dim=1, index=gather_idxs)

        self.state = self.state + state_deltas

        self.avg_val = self.avg_val*(1-self.alpha_avg.a) + input[:,2]*self.alpha_avg.a

        # record state histories
        self.state_hist.append(self.state.detach())
        self.avg_val_hist.append(self.avg_val.detach())
        self.state_diff_hist.append(state_diffs.detach())
        self.state_delta_hist.append(state_deltas.detach())

        return self.state
    
    def print_params(self):
        return '''Q Value Agent: \n\t α same = {}, fit: {} \n\t α diff = {}, fit: {} 
\t α avg = {}, fit: {} \n\t v init = {:.5f}, fit: {} \n\t κ diff = {:.5f}, fit: {}'''.format(
                  self.alpha_same.to_string(), self.alpha_same.requires_grad, self.alpha_diff.to_string(), self.alpha_diff.requires_grad,
                  self.alpha_avg.to_string(), self.alpha_avg.requires_grad, self.v_init.to_string(), self.v_init.requires_grad,
                  self.k_diff.item(), self.k_diff.requires_grad) 
